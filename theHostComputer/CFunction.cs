...
namespace theHostComputer
{
    class CFunction
    {
        ...
        private double[,] y;
        private double[,] x = new double[8, 1000000];
        ...
        //private double[,] xx = new double[12, 1000000]; 
        public double[,] UPDW = new double[12, 2]; //阈值
        public double[,] dy_original = new double[12, 100000];   //阈值去噪2
        public double[,] y_cidao = new double[12, 1000000];   //阈值去噪2
        public double[,] yshuzhi = new double[12, 1000000];   //阈值去噪2
        public int[,] xshuzhi = new int[12, 1000000];   //阈值去噪2
        public double[,] y_lvbo = new double[8, 1000000];
        //输入参数
        public void Function(double[,] y, int n, long m, double t)
        {
            this.y = y;
            this.tongdao_num = n;
        ...
        private double[,] Lvbo_fft()
        {
            ...
            double[] b = new double[27] { 0.0009, -0.0001, -0.0020, -0.0055, -0.0098, -0.0127, -0.0105, 0.0006, 0.0229, 0.0554, 0.0934, 0.1295, 0.1554, 0.1649, 0.1554, 0.1295, 0.0934, 0.0554, 0.0229, 0.0006, -0.0105, -0.0127, -0.0098, -0.0055, -0.0020, -0.0001, 0.0009 };
        ...
        public double[,] col_lip_1()
        {
            for (int i = 0; i < 8; i++)
            {
                for (int j = 0; j < this.ioriginal_num - 1; j++)
                {
                    x[i, j] = Math.Abs(y[i, j]);
                }
            }
            double y1temp = 0, y2temp = 0;
            int x1temp = 0, x2temp = 0;
            double[,] V = new double[this.tongdao_num * 40, this.ioriginal_num * 5];
            double[,] W = new double[this.tongdao_num, this.ioriginal_num * 5];
            /////// 小信号的显示
            ...
            for (int i = 0; i < this.ioriginal_num - 1; i++)
            {
                for (int j = 0; j < 8; j++)
                {
                    ...
                    b = 25 * min / 4 - 1 * max / 4;
                    if (x[j, i] > 0)
                    { x[j, i] = Math.Abs((-b + Math.Sqrt(Math.Abs(b * b - 4 * a * x[j, i]))) / (2 * a)); }
                    else
                    {
                        x[j, i] = x[j, i];
                    }
            ...
            /////先横向插值
            for (long line = 0; line < this.tongdao_num; line++)
            {
                for (int i = 0; i < this.ioriginal_num - 2; i++)
                {
                    y1temp = x[line, i];
                    y2temp = x[line, i + 1];
                    x1temp = i * 5;
                    ...
                    for (int j = 0; j < 5; j++)
                    {
                        W[line, x1temp + j] = y1temp + j * (y2temp - y1temp) / 4;
                    }
                }
                for (int j = 0; j < 5; j++)
                {
                    W[line, (this.ioriginal_num - 2) * 5 + j] = x[line, this.ioriginal_num - 2] + j * x[line, this.ioriginal_num - 2] / 4;
                }
            ...
            for (int i = 0; i < this.tongdao_num; i++)
            {
                for (int j = 0; j < this.ioriginal_num * 5 - 6; j = j + 1)
                {
                    double ssum = 0;
                    double mean = 0;
                    for (int ii = j; ii < j + 6; ii++)
                    {
                        ssum = ssum + W[i, ii];
                    }
                    mean = ssum / 6;
            ...
            for (int i = 0; i < this.tongdao_num; i++)
            {
                for (int j = (int)(this.ioriginal_num * 5 - 1); j > 6; j = j - 1)
                {
                    double ssum = 0;
                    double mean = 0;
                    for (int ii = j; ii > j - 6; ii--)
            ...
            /////////在做纵向差值
            for (long line = 0; line < this.ioriginal_num * 5; line++)
            {
                for (int i = 0; i < 20; i++)
                {
                    y1temp = 0;
                    y2temp = W[0, line];
                    x1temp = 0;
                    V[x1temp + i, line] = y1temp + i * (y2temp - y1temp) / 19;
                }
                for (int i = 0; i < this.tongdao_num - 1; i++)
                {
                    y1temp = W[i, line];
                    ...
                    for (int j = 0; j < 40; j++)
                    {
                        V[x1temp + j, line] = y1temp + j * (y2temp - y1temp) / 39;
                    }
                }
                for (int i = this.tongdao_num * 40 - 20; i < this.tongdao_num * 40; i++)
                {
                    y2temp = 0;
                    y1temp = W[this.tongdao_num - 1, line];
                    x1temp = this.tongdao_num * 10 - 5;
                    x2temp = this.tongdao_num * 10;
                    V[i, line] = y1temp + (i - (this.tongdao_num * 40 - 20)) * (y2temp - y1temp) / 19;
                }
            }
